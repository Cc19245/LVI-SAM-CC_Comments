...
void update()
{
    //// 获取滑动窗口中最后一帧的状态[P,V,Q]和零偏[Ba,Bg]
    //// 获取最新的加速度和角速度
    TicToc t_predict;
    ...
    //// 采用上面的新的参数，结合此时imu测量队列中剩余的imu测量，重新预测当前状态
...
//? added by lidar : 获取LiDAR里程计的位姿作为先验，注意这里的LiDAR里程计位姿是IMU频率下的
    ...
    while (ros::ok())
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++)
            {
                ...
                //? added by lidar : 特征点有LiDAR的深度信息辅助
            ...
            m_odom.lock();
            //; 注意这个odomQueue中的LIO里程计结果是IMU频率下的，所以只需要寻找和当前图像时间戳最近的那个
            //; 结果即可。而不是LiDAR频率的里程计（如果是这样就还需要利用IMU对其进行传播，得到当前图像的位姿）
            ...
            pubKeyframe(estimator);           //pub 最新关键帧位姿 及其 特征点各种坐标系的坐标 两个话题；（初始化及机器人静止时没有关键帧加入，则不会pub）；（初始化及机器人静止时没有关键帧加入，则不会pub）
        }
        m_estimator.unlock();
...
int main(int argc, char **argv)
{
    ...
    //// 创建odom注册器，用来获取LiDAR位姿并转换为camera位姿
    ...
    //; 注意这里已经是LIO的odom了，并且是IMU频率的odom
