...
/**
 * @brief  发布VINS-Mono中IMU回调函数里积分估计的位姿结果
 *  这个函数相比vins-mono中的原函数内容多了很多
 * 
 * @param[in] P 
 * @param[in] Q 
 * @param[in] V 
...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header, const int &failureId)
{
    ...
    odometry.header.frame_id = "vins_world";  // vins的world系
    ...
    tf::Quaternion q_odom_cam(Q.x(), Q.y(), Q.z(), Q.w());  //; R_WI
    ...
    //; R_WI * R_IL，这里其实就是为了转成ROS的前左上坐标系
    //; 这里确实只需要把旋转变一下， 因为位置速度不用变，这里只是把坐标系换一下而已
    tf::Quaternion q_odom_ros = q_odom_cam * q_cam_to_lidar;

    tf::quaternionTFToMsg(q_odom_ros, odometry.pose.pose.orientation);

    //? 重要：发给LIO的，让LIO得到VINS估计的里程计，注意旋转是ROS的前左上坐标系
    pub_latest_odometry_ros.publish(odometry);

    //; 发布TF变换：vins的world系 和 当前IMU的body系之间的坐标变换，注意姿态变成了ROS的前左上坐标系
    // TF of camera in  in ROS format (change rotation), used for depth registration
    ...
    //; params_camera.yaml中的align_camera_lidar_estimation参数
    // align camera and lidar estimation for visualization
    if (ALIGN_CAMERA_LIDAR_COORDINATE)
    {
        ...
        if (header.stamp.toSec() - last_align_time > 1.0)
        {
            try
            {
                ...
                //; 以IMU为基准的前左上坐标系就是base_link坐标系
                listener.lookupTransform("odom", "base_link", ros::Time(0), 
                    trans_odom_baselink);
                //; T_odom_base * T_base_vinsworld = T_odom_vinsworld
    ...
    {
        tf::Transform t_static = tf::Transform(
            tf::createQuaternionFromRPY(0, 0, M_PI), tf::Vector3(0, 0, 0));
        br.sendTransform(tf::StampedTransform(
            t_static, header.stamp, "odom", "vins_world"));
    }
}
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    transform.setRotation(q);
    br.sendTransform(
        tf::StampedTransform(transform, header.stamp, "vins_body", "vins_camera"));
