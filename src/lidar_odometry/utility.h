...
public:
    ...
    /**
     * @brief 此函数存在的目的是：把IMU坐标系下的测量数据，转成和LiDAR坐标系对齐的测量数据
     *  这里包括两部分：
     *  1.把IMU测量的 IMU坐标系 下的加速度和角速度，转成 LiDAR坐标系 下的加速度和角速度
     *  2.把IMU融合的 IMU坐标系在世界坐标系下的姿态R_W_I，转成 LiDAR坐标系在世界坐标系下的姿态R_W_L
     * 
     * @param[in] imu_in 
    ...
    sensor_msgs::Imu imuConverter(const sensor_msgs::Imu& imu_in)
    {
        ...
        //! 疑问：这里为什么是右乘？
        //; 解释：就是要转成R_W_L, 即LiDAR坐标系在世界坐标系下的位姿
